...
(deftest symbol-head-arity-0
  (is (= '[(foo)] (core/beme->forms "foo()"))))

(deftest symbol-head-arity-1
  (is (= '[(foo x)] (core/beme->forms "foo(x)"))))

(deftest symbol-head-arity-2
  (is (= '[(foo x y)] (core/beme->forms "foo(x y)"))))

(deftest symbol-head-arity-3+
  (is (= '[(foo a b c d)] (core/beme->forms "foo(a b c d)"))))

(deftest symbol-head-operator
  (is (= '[(+ 1 2 3)] (core/beme->forms "+(1 2 3)"))))

(deftest symbol-head-nested
  (is (= '[(a (b (c x)))] (core/beme->forms "a(b(c(x)))"))))
...
(deftest keyword-head-arity-0
  (is (= '(:foo) (first (core/beme->forms ":foo()")))))

(deftest keyword-head-arity-1
  (is (= '(:foo x) (first (core/beme->forms ":foo(x)")))))

(deftest keyword-head-arity-2
  (is (= '(:foo x y) (first (core/beme->forms ":foo(x y)")))))

(deftest keyword-head-arity-3+
  (is (= '(:foo a b c d) (first (core/beme->forms ":foo(a b c d)")))))

(deftest keyword-head-namespaced
  (is (= '(:ns/key x) (first (core/beme->forms ":ns/key(x)")))))

(deftest keyword-head-require-pattern
  (is (= '(:require [bar]) (first (core/beme->forms ":require([bar])")))))

(deftest keyword-head-import-pattern
  (is (= '(:import [java.util Date]) (first (core/beme->forms ":import([java.util Date])")))))
...
(deftest set-head-arity-1
  (let [form (first (core/beme->forms "#{:a :b}(x)"))]
    (is (set? (first form)))
    (is (= 'x (second form)))))

(deftest set-head-arity-0
  (let [form (first (core/beme->forms "#{:a}()"))]
    (is (set? (first form)))
    (is (= 1 (count form)))))
...
(deftest map-head-arity-1
  (let [form (first (core/beme->forms "{:a 1}(:a)"))]
    (is (map? (first form)))
    (is (= :a (second form)))))

(deftest map-head-arity-0
  (let [form (first (core/beme->forms "{:a 1}()"))]
    (is (map? (first form)))
    (is (= 1 (count form)))))
...
(deftest keyword-spacing-newline
  (is (= '(:k x) (first (core/beme->forms ":k\n(x)")))))
...
(deftest vector-spacing-newline
  (is (= '([x] 1) (first (core/beme->forms "[x]\n(1)")))))
...
(deftest set-spacing-adjacent
  (let [form (first (core/beme->forms "#{:a}(x)"))]
    (is (= 'x (second form)))))
...
(deftest symbol-eats-paren-in-call-args
  (testing "def(x (1 2 3)) — x eats the ("
    (is (= '[(def (x 1 2 3))] (core/beme->forms "def(x (1 2 3))")))))
...
(deftest vector-followed-by-symbol-call
  (testing "do([1 2 3] foo(x)) — vector does not eat symbol call"
    (is (= '[(do [1 2 3] (foo x))] (core/beme->forms "do([1 2 3] foo(x))")))))
...
(deftest pattern-defn-single-arity
  (is (= '[(defn greet [name] (str "Hello " name))]
         (core/beme->forms "defn(greet [name] str(\"Hello \" name))"))))

(deftest pattern-defn-multi-arity
  (let [result (first (core/beme->forms "defn(foo [x](x) [x y](+(x y)))"))]
...
(deftest pattern-ns-require
  (let [result (first (core/beme->forms "ns(my.app :require([clojure.string :as str]))"))]
    (is (= 'ns (first result)))
    (is (= 'my.app (second result)))
    (is (= :require (first (nth result 2))))))

(deftest pattern-ns-require-import
  (let [result (first (core/beme->forms "ns(my.app :require([clojure.string :as str]) :import([java.util Date]))"))]
    (is (= 4 (count result)))
    (is (= :require (first (nth result 2))))
    (is (= :import (first (nth result 3))))))

(deftest pattern-defprotocol
  (let [result (first (core/beme->forms "defprotocol(Drawable draw([this canvas]) bounds([this]))"))]
...
(deftest pattern-defrecord-with-impls
  (let [result (first (core/beme->forms "defrecord(Circle [r] Shape area([this] *(Math/PI r r)))"))]
...
(deftest pattern-let-with-calls
  (is (= '[(let [x (+ 1 2)] (* x x))]
         (core/beme->forms "let([x +(1 2)] *(x x))"))))

(deftest pattern-threading
  (is (= '[(-> x (foo 1) (bar 2))]
         (core/beme->forms "->(x foo(1) bar(2))"))))

(deftest pattern-try-catch
  (is (= '[(try (risky) (catch Exception e (handle e)))]
         (core/beme->forms "try(risky() catch(Exception e handle(e)))"))))
...
(deftest begin-end-basic-call
  (is (= '[(foo x y)] (core/beme->forms "foo begin x y end"))))

(deftest begin-end-zero-arity
  (is (= '[(foo)] (core/beme->forms "foo begin end"))))

(deftest begin-end-nested-parens
  (is (= '[(foo (bar x))] (core/beme->forms "foo begin bar(x) end"))))

(deftest begin-end-nested-begin-end
  (is (= '[(foo (bar x))] (core/beme->forms "foo begin bar begin x end end"))))

(deftest begin-end-with-spacing
  (testing "spacing irrelevant before begin"
    (is (= '[(f x)] (core/beme->forms "f  begin x end")))
    (is (= '[(f x)] (core/beme->forms "f\nbegin x end")))))
...
(deftest begin-end-keyword-head
  (is (= '(:require [bar]) (first (core/beme->forms ":require begin [bar] end")))))

(deftest begin-end-vector-head
  (is (= '([x] 1) (first (core/beme->forms "[x] begin 1 end")))))
...
(deftest begin-end-defn
  (is (= '[(defn greet [name] (str "Hello " name))]
         (core/beme->forms "defn begin greet [name] str(\"Hello \" name) end"))))

(deftest begin-end-let
  (is (= '[(let [x 1] (+ x 2))]
         (core/beme->forms "let begin [x 1] +(x 2) end"))))

(deftest begin-end-try-catch
  (is (= '[(try (risky) (catch Exception e (handle e)))]
         (core/beme->forms "try begin risky() catch begin Exception e handle(e) end end"))))
...
(deftest begin-end-unclosed-at-eof
  (testing "f begin x — missing end at EOF"
    (let [e (try (core/beme->forms "f begin x")
                 nil
  ...
  (testing "f begin — empty body, missing end"
    (is (thrown-with-msg? #?(:clj Exception :cljs js/Error)
          #"end"
          (core/beme->forms "f begin"))))
  (testing "nested begin without inner end"
    (is (thrown-with-msg? #?(:clj Exception :cljs js/Error)
          #"end"
          (core/beme->forms "f begin g begin x end")))))
